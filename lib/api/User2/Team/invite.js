      ...
      Model.Teams.findAll({
        ...
        if(leader.length >= 1) {
            Model.TeamInvites.findAll({
              ...
              include: [{model: Model.Students, as: 'Student', attributes:['Name']}]
          }).then(function(data) {
            r.status = statusCodes.SUCCESS;
            r.status.message = 'Successfully fetched the team Users';
            r.data = {'leader': leader, 'players': data};
            res.json(r);
          });
        }
        else {
          r.status = statusCodes.BAD_INPUT;
          r.data = "Team not found with this ID";
          res.json(r);
        }
    });
