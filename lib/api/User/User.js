...
router.post('/login',authenticateGmailToken,function(req,res) { //remove console.logs
  ...
  .then(function(data) {
    ...
    .catch(function(error) {
      ...
      // if theuser was already registered
      Model.Students.find({
        include: [{
...
router.get('/interestedEvents/',function(req,res){
  ...
  var userId = 1;            //change after testing
  //raw query used as the ttwo tables are not associated and no association is possible among them
  Model.sequelize.query("SELECT Events.Name FROM  Interests ,Events where Interests.CategoryId=Events.CategoryId and Interests.StudentId=?", {  replacements: [userId],type: Model.sequelize.QueryTypes.SELECT})
