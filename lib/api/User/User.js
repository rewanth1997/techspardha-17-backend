...
router.get('/search', function(req, res) {
  ...
  if(q) {
    var ids = new Set();
    Model.sequelize.query("SELECT distinct StudentDetails.Id FROM Students,StudentDetails WHERE (LOWER(Name) like LOWER(:query) OR PhoneNumber like :query OR RollNo like :query OR Email    like :query) AND StudentDetails.Id=Students.Id", {
      replacements: {
        query: '%' + q + '%'
      }
      ...
      Model.Students.findAll({
        ...
        var retUsers = new Set();
        users.map(function(user) {
          retUsers.add({
            Id: user.dataValues.Id,
            Name: user.dataValues.Name,
            RollNo: user.dataValues.Details.RollNo
          });
        });
        r.status = statusCodes.SUCCESS;
        r.data = Array.from(retUsers);
        return res.json(r);
      }).catch(function(err) {
...
router.get('/me/notifications',function(req,res) {
  ...
  var userId = req.user.id;
  var timeStamp = req.query.timeStamp.replace('+', ' ');
