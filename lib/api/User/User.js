...
router.get('/search', function(req, res) {
  ...
  if(q) {
    var ids = new Set();
    Model.sequelize.query("SELECT Id FROM Students WHERE LOWER(Name) like LOWER(:query) "+
    "UNION SELECT Id FROM StudentDetails WHERE PhoneNumber like :query", {
      replacements: {
        query: '%' + q + '%'
      }
...
router.get('/me/notifications',function(req,res) {
  ...
  attributes:['Id','status']
    ...
    Model.Notifications.findAll( {
      ...
      attributes:['EventId','Message','NotificationType']
    }).then(function(data) {
      globalData = data;
      r.status = statusCodes.SUCCESS;
