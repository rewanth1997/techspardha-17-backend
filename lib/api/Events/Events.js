...
router.get('/', function(req, res) {
  ...
  Model.Events.belongsTo(Model.Category);
  Model.Category.hasMany(Model.Events);
  if(query !== undefined ){
    Model.Events.findAll({
      attributes: ['Id','Name','Society'],
      include: [{ model: Model.Category,
                  attributes: ['id']}],
      having: ['MATCH(Events.Name,Description,Society) AGAINST(?)', query]  //ALTER TABLE Events ADD FULLTEXT(Name,Description,Society);//ALTER TABLE Category ADD FULLTEXT(Name) must be executed as fulltext is not getting created by sequelize
    }).then(function(data) {
